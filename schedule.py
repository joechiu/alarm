...
class Schedule(QDialog):
    ...
    def setData(self, i, new=0):
        ...
        self.id = data['id']
        # reserved ct and cb
        self.ct = self.menu.a.ct(i)
        self.cb = self.menu.a.cb(i)
        self.updateTime(r)
    ...
    def close(self):
        ...
        # self.menu.updateData(self.idx, self.data)
        print(self.menu.db.u(self.data))
        self.updateWorker()
        self.menu.reloadMenu()
        # self.menu.cbck(self.idx, c.T)
        self.hide()
    ...
    def updateWorker(self):
        i = self.idx
        self.updata()
        # self.menu.stopWork(i)
        self.menu.startWork(i)
        
    def updata(self):
        e = ('name', 'h', 'm', 's', 'd', 'c', 'r', 'a', 't', 'aud', 'msg', 'rgb', 'sz', 'oc')
        for f in e:
            self.menu.h[self.idx][f] = self.data[f]
    ...
    def timeChanged(self):
        ...
        self.updateWorker()
        self.menu.reloadMenu()
        self.menu.setTime(self.idx, self.menu.timeset(self.idx))
        self.Result()
        
    def nameChanged(self):
        lbl = self.menu.getLab(self.b1.text())
        self.menu.setCellWidget(self.idx, 2, lbl)        
        self.menu.reloadMenu()
    ...
    def seat(self):
        ...
        # x = p.x() + self.menu.width() / 3 
        # y = p.y() - (self.menu.height()+self.height())/len(self.menu.h)
        self.initSize()
        self.move(p.x(), p.y())        
